...
import { Course } from "@/app/(Server)/types/Course";
...
export default function CoursePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchCourseData(params?.id as string); // id ve lessonId'yi geçirin
        setCourse(data);
      } catch (error) {
      ...
      <div>
        ...
        <div className="p-3 bg-white rounded-sm">
          ...
          {course?.lessons?.map((item, index) => (
            <Link
              href={`/course-preview/${course.id}/lesson/${item.id}`}
              key={index}
            >
              <h2
